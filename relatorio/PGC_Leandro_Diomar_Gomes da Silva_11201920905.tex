...
\textbf{Continuidade de pesquisa}: O FeelFrame atua como elo entre a pesquisa científica do Prof. Nelson \cite{nascimento2024nelson} e a engenharia de software aplicada, demonstrando como resultados acadêmicos podem ser transpostos para produtos utilizáveis pela comunidade.
...
O UMAP (\textit{Uniform Manifold Approximation and Projection}) \cite{mcinnes2018umap} é um algoritmo avançado que reduz a complexidade dos dados de forma mais eficiente que o t-SNE. No FeelFrame, ele é usado opcionalmente no módulo de relatório para transformar todas as métricas de um frame (posição da cabeça, olhar e emoções) em gráficos 2D ou 3D. Isso agrupa visualmente os comportamentos semelhantes, permitindo identificar rapidamente os padrões de engajamento no vídeo.
